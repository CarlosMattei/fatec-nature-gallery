...
const images = import.meta.glob<string>("../fotos/**/*.jpg", {
  eager: true,
  import: "default",
});
...
export default function Gallery() {
  ...
  const validIndex =
    isNaN(photoIndex) || photoIndex < 0 || photoIndex >= photosData.length
      ? 0
      : photoIndex;
  ...
  const galleryImages = photosData.map((item) => ({
    src: getImageUrl(item.arquivo),
    alt: item.titulo,
  }));
  ...
  useEffect(() => {
    ...
    setNewBackgroundImage(
      getImageUrl(photosData[(nextIndex + 1) % photosData.length].arquivo),
    );
  ...
  useLayoutEffect(() => {
    if (!needsStyleReset.current) return;
      ...
      <section className="galleryStage" aria-label="Galeria de imagem">
        <div className="galleryContent">
          <div className="galleryPhotoColumn">
            <figure
              ref={primaryRef}
              className="photoFrame photoFrame--primary"
            ...
            <figure
              ref={secondaryRef}
              className="photoFrame photoFrame--secondary"
              aria-hidden="true"
            >
          ...
          <div className="galleryAside">
            <Link to="/map" className="backLink">
            ...
            <div className="buttonContainer">
              <button
                className="previous"
                onClick={() => handleNavigate(prevIndex, "prev")}
              >
                <span aria-hidden="true">
                  <IoChevronBackOutline size={24} stroke="var(--primary)" />
                </span>
              ...
              <button
                className="next"
                onClick={() => handleNavigate(nextIndex, "next")}
              >
                Próxima
                <span aria-hidden="true">
